...
const AppointmentFormIC = ({ doctorName, doctorSpeciality, onSubmit }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    console.log("Saving appointment to localStorage:", appointmentDetails);
    localStorage.setItem("appointmentData", JSON.stringify(appointmentDetails)); // Save appointment
    onSubmit(appointmentDetails); // Update state in parent
  };
  ...
  return (
    <form onSubmit={handleFormSubmit} className="appointment-form">
      ...
      <div className="form-group">
        <label htmlFor="timeSlot">Book Time Slot:</label>
        <input type="time" id="timeSlot" value={timeSlot} onChange={(e) => setTimeSlot(e.target.value)} required />
      </div>
      <button type="submit">Book Now</button>
    </form>
  );
